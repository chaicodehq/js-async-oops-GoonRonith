...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const avgStrikeRate = this.innings.reduce((sum, inning) => sum + inning.strikeRate, 0) / this.innings.length;
  ...
  getProfile() {
    ...
    return {
      ...
      role: "batsman",
      totalRuns: this.innings.reduce((sum, inning) => sum + inning.runs, 0),
      inningsPlayed: this.innings.length,
    };
  }
}
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const totalRuns = this.spells.reduce((sum, spell) => sum + spell.runsConceded, 0);
